...
class InvoiceTest {
  ...
  private static final ClientAccount ACCOUNT = new ClientAccount(
    ACCOUNT_NAME,
    CONTACT,
    ADDRESS
  );
  ...
  private static final TimeCard TIMECARD = new TimeCard(
    CONSULTANT,
    LocalDate.of(INVOICE_YEAR, INVOICE_MONTH, 2)
  );
  ...
  void setUp() {
    ...
    IntStream
      ...
      .forEach(i -> {
        invoice.addLineItem(
          new InvoiceLineItem(
            LocalDate.of(INVOICE_YEAR, INVOICE_MONTH, i),
            CONSULTANT,
    ...
    invoice.addLineItem(
      new InvoiceLineItem(
        LocalDate.of(1900, INVOICE_MONTH, 22),
        CONSULTANT_2,
        Skill.PROJECT_MANAGER,
        10
      )
    ...
    invoice.addLineItem(
      new InvoiceLineItem(
        LocalDate.of(INVOICE_YEAR, 5, 22),
        CONSULTANT_2,
        Skill.PROJECT_MANAGER,
        10
      )
  ...
  void getReportGetString() throws IOException {
    String expected = new String(
      Objects
        .requireNonNull(getClass().getClassLoader().getResourceAsStream(INVOICE_EXAMPLE))
        .readAllBytes()
    );
  ...
  void getGetString() {
    String expected =
      "Invoice{client=ClientAccount{name='Account 1', contact=Bob, Jim NMN, address=125 main\n" +
