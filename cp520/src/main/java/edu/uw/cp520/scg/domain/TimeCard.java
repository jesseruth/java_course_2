...
public final class TimeCard implements Comparable<TimeCard>, Serializable {
   ...
   */
  private static final String LINE_HEADER_FORMAT = String.format(
    "%-28s %-10s  %5s  %s%n" +
    "---------------------------  ----------  -----  --------------------%n",
    "Account",
    "Date",
  ...
  public List<ConsultantTime> getBillableHoursForClient(String clientName) {
    return consultingHours
      .stream()
      .filter(consultantTime ->
        clientName.equals(consultantTime.getAccount().getName()) &&
  ...
  public String toReportString() {
    String title = String.format(
      "Consultant: %s              Week Starting: %s%n%n",
      getConsultant(),
      getWeekStartingDay()
    );

    String billable = consultingHours
      .stream()
      .filter(hours -> hours.isBillable())
      .map(hours -> {
        return String.format(
          "%-28s %s  %5s  %-20s",
          hours.getAccount().getName(),
          hours.getDate(),
          hours.getHours(),
          hours.getSkillType()
        );
    ...
    String nonBillable = consultingHours
      .stream()
      .filter(hours -> !hours.isBillable())
      .map(hours -> {
        return String.format(
