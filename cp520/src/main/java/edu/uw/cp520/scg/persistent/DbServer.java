...
public final class DbServer {
     ...
     */
    private static final String GET_CONSULTANT_ID =
        "SELECT id FROM consultants WHERE last_name = ? AND first_name = ? AND middle_name = ?";
    /**
     * Get all consultants
     */
    private static final String GET_CONSULTANTS =
        "SELECT last_name, first_name, middle_name FROM consultants";
    /**
     * add a consultant.
     */
    private static final String ADD_CONSULTANT =
        ...
        try (Connection conn = getConnection()) {
            ...
            while (rs.next()) {
                ...
                PersonalName contact = new PersonalName(
                    ...
                    rs.getString("contact_middle_name"),
    ...
    public void addTimeCard(TimeCard timeCard) throws SQLException {
        ...
        try (Connection conn = getConnection()) {
            PreparedStatement getConsultantID = conn.prepareStatement(
                GET_CONSULTANT_ID,
                Statement.RETURN_GENERATED_KEYS
            );
            ...
            PreparedStatement addTimeCard = conn.prepareStatement(
                ADD_TIMECARD,
                Statement.RETURN_GENERATED_KEYS
            );
            ...
            for (ConsultantTime consultantTime : timeCard.getConsultingHours()) {
                ...
                if (consultantTime.isBillable()) {
                    ...
                    nonBillStatement.setString(
                        4,
                        String.valueOf(consultantTime.getHours())
                    );
                    ...
                    int consultantTimeID = rs.getInt(1);
                    log.info(
                        "Added Non-billable consultant Time ID:{}",
                        consultantTimeID
                    );
                }
            }
    ...
    public Invoice getInvoice(ClientAccount client, Month month, int year)
        ...
        try (
            Connection conn = getConnection();
            PreparedStatement stmnt = conn.prepareStatement(GET_INVOICE)
        ) {
            final LocalDate START_DATE = LocalDate.of(year, month, 1);
            final LocalDate END_DATE = START_DATE.plusDays(
                START_DATE.lengthOfMonth() - 1
            );
